import type { Project, SurfaceConnection } from '@/types';

export type ConnectionFormValues = Pick<
  SurfaceConnection,
  | 'connectionType'
  | 'angleDeg'
  | 'jointAtConnectionMm'
...
export const getConnectionTypeDefaults = (
  ...
  switch (connectionType) {
    ...
    case 'buttJoint': {
      const overlapMm = inferredThicknessMm > 0
        ? inferredThicknessMm
        : FALLBACK_BUTT_JOINT_OVERLAP_MM;
      return {
        connectionType,
        angleDeg: 90,
        jointAtConnectionMm: 0,
